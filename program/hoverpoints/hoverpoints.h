...
public slots:
    ...
    void zmiana_trybu(bool obrysNerwu) ;
	void reset();

signals:
    void zmianaPunktow(double x0, double x1);
...
private:
    inline QRectF pointBoundingRect(int i, const QPolygonF * ) const;
    void movePoint(QPointF &oldPos, const QPointF &newPos, bool emitChange = true);

    QWidget *m_widget;

    QPolygonF m_points_nerw;
    QPolygonF m_points_brzeg;
    QRectF m_bounds;
    PointShape m_shape;
    SortType m_sortType;
 ...
 //   QVector<uint> m_locks;
    ...
    QPen m_pointPen;
    QPen m_endPointPen, m_nerwPen, m_brzegPen;

    QBrush m_nerwBrush;
    QBrush m_brzegBrush;
    QBrush m_endPointBrush;
    QPen m_connectionPen;
	QPen m_connection2Pen;
...
inline QRectF HoverPoints::pointBoundingRect(int i, const QPolygonF * poly) const
{
    QPointF p = poly->at(i);
